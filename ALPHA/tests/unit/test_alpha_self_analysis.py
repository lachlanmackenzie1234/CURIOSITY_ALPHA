...
class TestALPHASelfAnalysis(unittest.TestCase):
    ...
    def test_incremental_analysis(self):
        ...
        self.assertGreaterEqual(sum(stats2.values()), sum(stats1.values()))
    ...
    def test_error_handling(self):
        ...
        async def test_invalid_inputs():
            ...
            results = await self.analyzer.analyze_codebase(str(Path(self.test_dir) / "empty"))
            self.assertEqual(results["files_analyzed"], 0)

            # Test invalid file content
            results = await self.analyzer.analyze_codebase(str(Path(self.test_dir) / "invalid"))
            self.assertLess(results["translation_effectiveness"]["success_rate"], 1.0)
