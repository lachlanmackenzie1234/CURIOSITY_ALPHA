...
class TestBinaryMapping(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.prime_pattern = NaturalPattern(
            principle_type="prime", sequence=[2, 3, 5, 7, 11], confidence=0.9
        )
    ...
    def test_pattern_to_binary(self):
        ...
        mapping = self.mapper.map_to_binary(self.fibonacci_pattern, self.context)
    ...
    def test_binary_to_pattern(self):
        ...
        original_mapping = self.mapper.map_to_binary(self.fibonacci_pattern, self.context)

        # Map back to pattern
        recovered_pattern = self.mapper.map_from_binary(
            original_mapping.binary_form, original_mapping.encoding_type
        )
    ...
    def test_encoding_types(self):
        ...
        for encoding_type in self.mapper.ENCODING_TYPES:
            ...
            # Verify encoding
            self.assertEqual(mapping.encoding_type, encoding_type)
            self.assertIsNotNone(mapping.binary_form)
    ...
    def test_pattern_preservation(self):
        ...
        # Map back to pattern
        recovered = self.mapper.map_from_binary(mapping.binary_form, mapping.encoding_type)

        # Verify properties preserved
        self.assertEqual(recovered.principle_type, self.fibonacci_pattern.principle_type)
        self.assertGreater(recovered.confidence, 0.7)
    ...
    def test_pattern_markers(self):
        ...
        # Verify markers
        self.assertTrue(mapping.binary_form.startswith(self.mapper.PATTERN_START))
        self.assertTrue(mapping.binary_form.endswith(self.mapper.PATTERN_END))

    def test_error_handling(self):
        """Test error handling in mapping process."""
        # Test with invalid pattern
        invalid_pattern = NaturalPattern(principle_type="invalid", sequence=[], confidence=0.0)

        # Attempt mapping
        mapping = self.mapper.map_to_binary(invalid_pattern, self.context)
    ...
    def test_context_influence(self):
        ...
        mapping1 = self.mapper.map_to_binary(self.fibonacci_pattern, context1)
        mapping2 = self.mapper.map_to_binary(self.fibonacci_pattern, context2)

        # Verify context influence
        self.assertNotEqual(mapping1.binary_form, mapping2.binary_form)
