...
class TestBinaryFoundation(unittest.TestCase):
    ...
    def test_pattern_encoding(self):
        ...
        self.assertEqual(
            bytes(self.binary._data[:4]),
            Binary.MARKER_PATTERN
        )
    ...
    def test_pattern_header_optimization(self):
        ...
        # Verify header size
        header_size = (
            len(Binary.MARKER_PATTERN) +  # Pattern marker
            4 +  # Pattern type
            4 +  # Confidence (float)
            4    # Length (uint32)
        )
