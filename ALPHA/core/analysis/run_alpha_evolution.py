...
class PatternEvolution:
    ...
    async def run_evolution_cycle(self) -> None:
        ...
        try:
            ...
            # Update metrics
            self.metrics.current_confidence = analysis_report.get(
                'average_confidence', 0
            )
            self.metrics.current_complexity = analysis_report.get(
            ...
            for rec in recommendations:
                ...
                msg = (
                    f"Evolution recommendation for {rec['file']}: "
                    f"{rec['message']}"
                )
                self.logger.info(msg)
            ...
            self.logger.info(
                f"Patterns/hour: {summary['patterns_per_hour']:.2f}"
            )
            self.logger.info(
                f"Success rate: {summary['improvement_success_rate']:.2%}"
            )
            self.logger.info(f"Error rate: {summary['error_rate']:.2%}")
            self.logger.info(
                f"Current confidence: {summary['current_confidence']:.2f}"
            )
            self.logger.info(
                f"Current complexity: {summary['current_complexity']:.2f}"
            )
