...
from ..patterns.binary_mapping import BinaryMapping, PatternMapper
...
class PatternFirstTranslator:
    ...
    def translate_from_binary(self, binary: bytes, preserve_patterns: bool = True) -> Optional[str]:
    ...
    def _identify_patterns(self, data: np.ndarray) -> List[TranslationUnit]:
        ...
        try:
            ...
            for i in range(0, len(data), self.pattern_window - self.overlap_size):
                ...
                patterns: Dict[str, NaturalPattern] = {}
                pattern = self.hierarchy.detect_natural_pattern(window)
    ...
    def _create_mappings(self, units: List[TranslationUnit]) -> List[TranslationUnit]:
    ...
    def _optimize_preservation(self, units: List[TranslationUnit]) -> List[TranslationUnit]:
        ...
        try:
            ...
            while i < len(units):
                ...
                if unit.patterns and unit.preservation_score > self.preservation_threshold:
                    ...
                    if related_units:
                        # Merge related units
                        merged = self._merge_units([unit] + related_units)
    ...
    def _extract_units(self, binary: bytes) -> List[TranslationUnit]:
        ...
        try:
            ...
            while pos < len(binary):
                if binary[pos] == self.mapper.PATTERN_START and pos + 1 < len(binary):
                    ...
                    pattern = self.mapper.map_from_binary(section, encoding_type)
                    if pattern:
                        unit = TranslationUnit(
                            content=section,
                            patterns={"p0": pattern},
                    ...
                    next_pattern = binary.find(self.mapper.PATTERN_START, pos + 1)
                    if next_pattern == -1:
                        next_pattern = len(binary)

                    section = binary[pos:next_pattern]
                    unit = TranslationUnit(content=section, patterns={}, mappings={})
                    units.append(unit)
    ...
    def _recover_patterns(self, units: List[TranslationUnit]) -> List[TranslationUnit]:
    ...
    def _optimize_recovery(self, units: List[TranslationUnit]) -> List[TranslationUnit]:
    ...
    def _generate_content(self, units: List[TranslationUnit]) -> Optional[str]:
        ...
        try:
            ...
            for unit in units:
                if unit.patterns and unit.preservation_score > self.preservation_threshold:
                    ...
                    best_pattern = max(unit.patterns.values(), key=lambda p: p.confidence)
                    sequence = best_pattern.sequence if best_pattern.sequence else [0]
                    # Convert sequence to bytes
                    byte_data = [int(x) % 256 for x in sequence]
    ...
    def _are_units_related(self, unit1: TranslationUnit, unit2: TranslationUnit) -> bool:
        ...
        try:
            ...
            for p1 in unit1.patterns.values():
                for p2 in unit2.patterns.values():
                    ...
                    if (
                        p1.principle_type in self.hierarchy.relationships
                        and p2.principle_type in self.hierarchy.relationships[p1.principle_type]
                    ):
                        return True
    ...
    def _merge_units(self, units: List[TranslationUnit]) -> TranslationUnit:
        """Merge related translation units."""
        try:
            if not units:
                return TranslationUnit(content=b"", patterns={}, mappings={})
            ...
            for unit in units:
                ...
                preservation_score = max(preservation_score, unit.preservation_score)
            ...
            return units[0] if units else TranslationUnit(content=b"", patterns={}, mappings={})
