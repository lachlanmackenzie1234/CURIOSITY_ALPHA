...
class MemoryMonitor:
    ...
    def _calculate_stability(self) -> float:
        ...
        try:
            ...
            # Combine factors
            stability = cpu_stability * 0.3 + memory_stability * 0.3 + error_stability * 0.4
    ...
    def _check_system_health(self) -> None:
        ...
        try:
            ...
            if self.metrics.error_rate > self.error_threshold:
                self.logger.error(f"High error rate detected: {self.metrics.error_rate:.2%}")
                self._trigger_recovery()

            if self.metrics.stability_score < self.warning_threshold:
                self.logger.warning(f"Low stability detected: {self.metrics.stability_score:.2f}")

            if self.metrics.health_score < self.warning_threshold:
                self.logger.warning(f"Poor system health: {self.metrics.health_score:.2f}")
    ...
    def _handle_monitoring_error(self) -> None:
        ...
        try:
            ...
            if self.recovery_attempts > 3:
                self.logger.critical("Multiple monitoring errors. Initiating system reset.")
    ...
    def _save_metrics(self) -> None:
        ...
        try:
            if self.metrics.metrics_history:
                ...
                self.logger.info(f"Saving {len(self.metrics.metrics_history)} metric records")
